    ...
    <>
      <div className="bg-gray-100 p-6 flex justify-between items-center h-20 sticky top-0 z-10">
        ...
        >
          ...
          <h2 className="border-b-2 border-black ml-1 w-14 hover:w-28 transition-all">
            <ReactLink to={"/"}>HypeCart</ReactLink>
          </h2>
        </motion.div>
        <motion.div
          initial={{ x: 0, opacity: 0 }}
        ...
        >
          <li className="md:mx-4 px-4 hover:text-gray-500 hover:border-b-2 border-black cursor-pointer transition-all list-none">
            <ReactLink to={"/"}>Home</ReactLink>
          </li>
          <li className="md:mx-4 px-4 hover:text-gray-500 hover:border-b-2 border-black cursor-pointer transition-all list-none">
            <ReactLink to={"/category"}>Products</ReactLink>
          </li>
          {user ? (
            <>
              <button
        ...
        <div
          className="md:hidden z-10 duration-150 transition-all"
          onClick={() => setNav(!nav)}
        >
          {nav ? (
        ...
        {nav && (
          <ul className="flex flex-col justify-center items-center absolute top-0 right-0 h-[564px] w-40 bg-gray-400 text-black text-xl bg-opacity-90 gap-y-5">
            ...
            {user && (
              <ReactLink
                to={"/cart"}
                className="cursor-pointer w-5 h-5 hover:border-b-2 border-black transition-all"
                onClick={() => setNav(!nav)}
              >
                <ShoppingCartIcon />
              </ReactLink>
